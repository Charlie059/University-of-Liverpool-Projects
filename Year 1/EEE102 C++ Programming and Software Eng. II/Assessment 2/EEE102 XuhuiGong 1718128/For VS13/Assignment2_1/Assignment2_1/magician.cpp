...
magician::magician(int lv_in, string name_in)
{
	...
	Intelligence = 25 + 2 * (LV - 1);  // Intelligence increases 2 points per level
...
bool magician::attack(player &p)
{
	...
	{
		HPtemp = (int)((1.0*AP / p.DP)*Intelligence * 5 / (rand() % 4 + 10));		// opponent's HP decrement calculated based their AP/DP and Intelligence, and uncertain chance
	...
	// Normal attack
	HPtemp = (int)((1.0*AP / p.DP)*AP * 4 / (rand() % 5 + 10));
...
bool magician::specialatt(player &p)
{
	if (MP < 40)
	{
	...
	else
	{
		...
		//double hit=1;			
		//srand(time(NULL));		
		HPtemp = (int)((AP + 0.4*Intelligence)*1.2 + 20);		// not related to opponent's DP but related to the Intelligence
