...
archer::archer(int lv_in, string name_in)
{
	...
	speed = 25 + 2.5 * (LV - 1);		// speed increases 2 points per level
...
bool archer::attack(player &p)
{
	...
	// Normal attack
	HPtemp = (int)((1.0*speed / p.DP)*AP * 5 / (rand() % 4 + 10));//The normal attack related to the speed and dp
...
bool archer::specialatt(player &p)
{
	...
	else
	{
		...
		//double hit=1;			
		//srand(time(NULL));		
		HPtemp = (int)(speed + 28);		// not related to opponent's DP but speed
